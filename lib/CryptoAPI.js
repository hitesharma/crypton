...
class CryptoAPI {
  ...
  async getHighData(coinOp) {
    try {
      ...
      res.data.forEach((coin) => {
        output += `${coin.rank.magenta} - ${coin.symbol.yellow} (${coin.name.cyan}) | High: ${coin.high.green} | Timestamp: ${coin.high_timestamp.grey}\n`;
      });
    ...
    try {
      ...
      res.data.forEach((coin) => {
        output += `${coin.rank.magenta} - ${coin.symbol.yellow} (${
          coin.name.cyan
        }) | Current Supply: ${coin.circulating_supply.green} | Max Supply: ${
